...
void main() {
  group('CompareUseCase', () {
    ...
    group('compare', () {
      ...
      test('even-count median averages two middle values', () async {
        final product = await repository.createProvisionalProduct(
          '4901234567890',
        );

        // 4 observations: sorted [100, 200, 800, 900] → median = (200+800)/2 = 500
        await insertObservationsSpaced(
          repository,
          productId: product.id,
          prices: [100, 800, 900, 200],
        );
        ...
        // (600-500)/500 = +20% → expensive (>= +10%)
        expect(result.label, domain.ComparisonLabel.expensive);
      });

      test('odd-count median picks exact middle value', () async {
        final product = await repository.createProvisionalProduct(
          '4901234567890',
        );

        // 5 observations: sorted [100, 200, 500, 800, 900] → median = 500
        await insertObservationsSpaced(
          repository,
          productId: product.id,
          prices: [100, 800, 500, 900, 200],
        );
