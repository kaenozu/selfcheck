...
final RegExp _unitPriceContextPattern = RegExp(
  r'(?:[0-9]+\s*(?:g|kg|ml|l|個|本|枚|袋|パック)\s*(?:当たり|あたり)|/\s*[0-9]+\s*(?:g|kg|ml|l|個|本|枚|袋|パック))',
  caseSensitive: false,
);
...
/// ¥198" or "¥198/100g" are rejected entirely so they cannot outrank the
/// actual shelf price. Among equally-ranked bare candidates, the larger OCR
/// region wins because shelf-label prices are typically the dominant text.
PriceCandidate? parsePriceText(Iterable<({String text, Rect region})> lines) {
  _ParsedPrice? best;

  for (final line in lines) {
    if (_unitPriceContextPattern.hasMatch(line.text)) {
      continue;
    }
