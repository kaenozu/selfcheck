...
The latest CI run for this branch (`33299273787`) completed successfully. CI success means the automated suite passed; it does not convert PENDING device acceptance into PASS.
...
|---|---|---|---|
| AC-01 | バーコード+価格同時 → 3秒以内に結果 | ⏳ PENDING | Compare/use-case timing is not camera-to-result timing. Real camera/JAN/OCR is implemented in #1 / PR #23, but physical-device end-to-end latency still needs measurement. |
...
| AC-05 | 同一SKU+同一価格の短時間連続 → 重複防止 | 🔧 FIX IN PROGRESS | Fixed-bucket tests were insufficient. #7 / PR #31 adds a rolling five-minute query and the 10:04:59 → 10:05:01 boundary regression. Keep non-PASS until that change lands and CI passes. |
...
| AC-07 | カメラ画像がデータ領域に残らない | ⚠️ PARTIAL | Static/data-model evidence shows no image column/path and PR #23 processes frames in memory. Physical-device filesystem/cache inspection is still required. |
...
| AC-09 | 価格未検出 → 認識継続 | ⚠️ PARTIAL | Coordinator lifecycle tests cover state recovery, but real camera/OCR continuation requires device acceptance with PR #23. |
| AC-10 | 中央値→差額・差率・ラベル正計算 | 🔧 FIX IN PROGRESS | Existing odd-count tests pass, but the prior report missed the even-count bug. #5 / PR #26 adds the mathematical even-count median regression. Keep non-PASS until that change lands. |
| AC-11 | 過去0件 → 初回価格表示 | ✅ PASS (automated) | Use-case/result tests verify `firstPrice` with no label. |
| AC-12 | 再スキャン優先（安全性） | ✅ PASS (automated) | `ac_state_machine_validation_test.dart` proves a transient wrong OCR value is not persisted and only the later stable price is saved. |

---

## Production-path tests added for #10

`test/integration/ac_state_machine_validation_test.dart` now provides concrete state-machine evidence for the safety-critical paths that were previously represented by weaker tests:
...
These tests exercise the application state machine and real Drift-backed test repository. They still use deterministic test adapters rather than a physical camera, so they cannot satisfy the device-only gates listed above.
